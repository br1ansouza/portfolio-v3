import type { Certificate } from '../types';

export const certificates: Certificate[] = [
  {
    name: {
      en: 'Automated Testing with Playwright',
      pt: 'Testes Automatizados com Playwright',
    },
    institution: 'Udemy',
    date: '2025',
    link: 'https://drive.google.com/file/d/16YwrTyjy3eQs3uU3RTltKDhVvj05phc_/view?usp=drive_link',
  },
  {
    name: {
      en: 'Computing Environments and Connectivity',
      pt: 'Ambientes Computacionais e Conectividade',
    },
    institution: 'Unisul',
    date: '2025',
    link: 'https://drive.google.com/file/d/151hXSNY0QEshYFQBHKtpyaFK0k0wdHk9/view?usp=sharing',
  },
  {
    name: { en: 'Full Stack Development', pt: 'Desenvolvimento Full Stack' },
    institution: 'SENAI / LAB365',
    date: '2025',
    link: 'https://drive.google.com/file/d/1fQTwMoMofadk9GqVy6hvvqSwRRiRbimX/view?usp=drive_link',
  },
  {
    name: { en: 'Full Stack · Module 3', pt: 'Full Stack · Módulo 3' },
    institution: 'LAB365',
    date: '2025',
    link: 'https://drive.google.com/file/d/1BjzgSB1mwL47Iydj0eKe78qbuIFod89j/view?usp=sharing',
  },
  {
    name: { en: 'Back-end · Module 2', pt: 'Back-end · Módulo 2' },
    institution: 'LAB365',
    date: '2025',
    link: 'https://drive.google.com/file/d/1iL9Pg_g_AwZO15JNWQqso0OJFRXS73Ls/view?usp=sharing',
  },
  {
    name: { en: 'Front-end · Module 1', pt: 'Front-end · Módulo 1' },
    institution: 'LAB365',
    date: '2025',
    link: 'https://drive.google.com/file/d/1-XHxEg9cCRRAyv5e_1Io2noM_zjEIgb1/view?usp=sharing',
  },
  {
    name: {
      en: 'Compact and Complete Web Development',
      pt: 'Desenvolvimento Web Compacto e Completo',
    },
    institution: 'Udemy',
    date: '2023',
    link: 'https://drive.google.com/file/d/1F7iHWIBZL1z_ESQ8g4nn_zHeedvTjOjF/view?usp=sharing',
  },
  {
    name: { en: 'Network Architecture', pt: 'Arquitetura de Redes' },
    institution: 'Udemy',
    date: '2023',
    link: 'https://drive.google.com/file/d/1u-_zNT4k-cAbBc5vBg73ONCtUj-3-jxD/view?usp=sharing',
  },
  {
    name: { en: 'Proxmox · Professional', pt: 'Proxmox · Professional' },
    institution: 'Udemy',
    date: '2023',
    link: 'https://drive.google.com/file/d/1xQC7fbBYRnFmgEG8sqMlX8-rObgjOuQ5/view?usp=sharing',
  },
  {
    name: { en: 'Programming Fundamentals', pt: 'Fundamentos de Programação' },
    institution: 'Udemy',
    date: '2022',
    link: 'https://drive.google.com/file/d/1AUksurChYYHK9Xk61tbU8iOD1BNWe0ad/view?usp=sharing',
  },
  {
    name: { en: 'Git', pt: 'Git' },
    institution: 'Udemy',
    date: '2022',
    link: 'https://drive.google.com/file/d/1Kgrh015VJbjAs9087GGBf9vQxZEhaNFT/view?usp=sharing',
  },
];
